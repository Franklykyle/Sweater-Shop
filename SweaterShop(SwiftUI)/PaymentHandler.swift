...
//  SweaterShop(SwiftUI)
//
//  Created by Kyle Franklin on 4/18/22.
//
...
class PaymentHandler: NSObject {
    ...
    func shippingMethodCalculator() -> [PKShippingMethod] {
        ...
        let shippingStart = calander.date(byAdding: .day, value: 5, to: today)
        let shippingEnd = calander.date(byAdding: .day, value: 10, to: today)
        
        if let shippingEnd = shippingEnd, let shippingStart = shippingStart {
            let startComponents = calander.dateComponents([.calendar, .year, .month, .day], from: shippingStart)
            let endComponents = calander.dateComponents([.calendar, .year, .month, .day], from: shippingEnd)
    ...
    func startPayment(products: [Product], total: Int, completion: @escaping PaymentCompletionHandler) {
        ...
        paymentRequest.merchantIdentifier = "merchant.com.designcode.swestershopapp"
